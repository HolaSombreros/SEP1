...
public class DetailsAndEditTaskController
{
    ...
    try
    {
      ...
      else if (timeRegisterInput.getText().equals("") && selectedTeamMember != null)
      {
        throw new IllegalStateException(
            "Please specify the number of hours #" + selectedTeamMember.getIdProperty().get() + " " + selectedTeamMember.getNameProperty().get() + " worked for");
      }
      else if (timeRegisterInput.getText().equals("") && selectedTeamMember == null)
      {
      ...
      else
      {
        ...
        if (confirmation("registertime"))
        {
          ...
          alert.showAndWait();
          model.editTask(task, task.getTitle(), task.getEstimatedTime(), task.getStartingDate(), task.getDeadline(), task.getStatus(), task.getResponsibleTeamMember(), hoursWorked,
              teamMember);
          errorLabel.setText("");
          timeRegisterInput.setText("");
          totalHoursWorked.setText(String.valueOf(task.getTimeRegistration().getHoursWorked()));
  ...
  {
    Task task = model.getProjectList().getProjectByID(viewState.getSelectedProject()).getProjectRequirementList().getRequirementById(viewState.getSelectedRequirement())
        .getTaskList().getTaskById(viewState.getSelectedTask());
    if (confirmation("remove"))
    {
      model.removeTask(task.getRelatedRequirement(), task);
  ...
  private boolean confirmation(String id)
  {
    ...
    switch (id)
    {
      case "registertime":
        ...
        alert.setHeaderText("Registering time for #" + teamTable.getSelectionModel().getSelectedItem().getIdProperty().get() + " " + teamTable.getSelectionModel().getSelectedItem()
            .getNameProperty().get());
        break;
      case "edit":
